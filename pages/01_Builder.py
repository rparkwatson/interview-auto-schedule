# 01_Builder.py (Step 1: Upload → Convert → Edit → Generate)
import io
import re
import hashlib
...
def _guess_name_column(df: pd.DataFrame) -> str | None:
    ...
    preferred = ["interviewer", "interviewer name", "name", "full name", "af name", "adcom", "adcom name"]
...
def _build_pre_table_from_names(names: pd.Series) -> pd.DataFrame:
    out = pd.DataFrame(
        {"Interviewer": names.astype(str).str.strip().dropna().unique()}
    )
...
def _inject_preassigned_counts(
    ...
    def aliases(base_header: str, extras: list[str] | None) -> set[str]:
        base = {
            _norm(base_header), "interviewer", "interviewer name", "name",
            "interviewer_name", "interviewername", "adcom", "adcom name"
        }
        if extras:
    ...
    count_aliases = {_norm(count_header), "pre_assigned_count", "pre-assigned count", "pre assigned count"}

    candidate_ws = [wb[sheet_name]] if sheet_name and sheet_name in wb.sheetnames else [wb[s] for s in wb.sheetnames]
    ...
    pre_df["Pre_Assigned_Count"] = pd.to_numeric(pre_df["Pre_Assigned_Count"], errors="coerce").fillna(0).astype(int)
...
def _clear_everything():
    for k in [
        "primary", "adcom",
        "primary_bytes", "adcom_bytes",     # NEW cached bytes
        "master_df", "max_df", "adcom_df",
        "reg_pre_df", "adcom_pre_df",
        "reg_name_col", "adcom_name_col",
        "formatted_xlsx", "pre_stats",
        "last_gen_fp",                      # NEW stored fingerprint
        "proceeded",
        # any widget keys to reset explicitly:
        "reg_fill_val", "adcom_fill_val", "max_editor",
        "reg_pre_editor", "adcom_pre_editor",
        "reg_name_col_select", "adcom_name_col_select",
    ]:
...
def _invalidate_if_inputs_changed(show_message: bool = True):
    ...
    if st.session_state.get("formatted_xlsx") and prev_fp and current_fp != prev_fp:
        ...
        if show_message:
            st.info("Inputs changed (Max_Pairs or Pre_Assigned_Counts). Please click **Generate Workbook for Step 2** again.")
...
if "master_df" in st.session_state and "adcom_df" in st.session_state:
    ...
    with c1:
        reg_name_col = st.selectbox(
            ...
            index=max(0, list(st.session_state.master_df.columns).index(st.session_state.reg_name_col)) if st.session_state.reg_name_col in st.session_state.master_df.columns else 0,
    ...
    with c2:
        adcom_name_col = st.selectbox(
            ...
            index=max(0, list(st.session_state.adcom_df.columns).index(st.session_state.adcom_name_col)) if st.session_state.adcom_name_col in st.session_state.adcom_df.columns else 0,
    ...
    if reg_name_col != st.session_state.reg_name_col:
        ...
        st.session_state.reg_pre_df = _build_pre_table_from_names(st.session_state.master_df[reg_name_col])
    if adcom_name_col != st.session_state.adcom_name_col:
        st.session_state.adcom_name_col = adcom_name_col
        st.session_state.adcom_pre_df = _build_pre_table_from_names(st.session_state.adcom_df[adcom_name_col])
...
if has_build:
    ...
    if valid_build:
        ...
        st.warning("Edits detected after the last build. Click **Generate Workbook for Step 2** to rebuild.")
    ...
    st.download_button(
        ...
        help=None if valid_build else "Edits detected. Click “Generate Workbook for Step 2” to rebuild."
    )

    # Step 2 button (disabled if invalid)
    if valid_build:
        if st.button("➡️ Open Step 2 — Scheduler", type="secondary", use_container_width=True):
